...
async function buildMainPackage() {
  ...
  await copyFile(
    "bin/kuuzuki-launcher.cmd",
    join(distDir, "bin", "kuuzuki.cmd"),
  );

  // Copy postinstall script
  await copyFile(
    "scripts/postinstall-prod.mjs",
...
async function buildPlatformPackage(platform: (typeof platforms)[0]) {
  ...
  const tuiBinaryName =
    platform.os === "windows" ? "kuuzuki-tui.exe" : "kuuzuki-tui";

  // Try different naming conventions for TUI binaries
  const tuiBinaryPaths = [
    join(
      "binaries",
      `kuuzuki-tui-${platform.os}-${platform.arch}${platform.os === "windows" ? ".exe" : ""}`,
    ),
    join(
      "binaries",
      `kuuzuki-tui-${platform.os}${platform.os === "windows" ? ".exe" : ""}`,
    ),
    join(
      "../../tui",
      `kuuzuki-tui-${platform.os}-${platform.arch}${platform.os === "windows" ? ".exe" : ""}`,
  ...
  if (!tuiCopied) {
    console.log(
      `  ℹ️  No TUI binary found for ${platform.os}-${platform.arch}`,
    );
  }
  ...
  // Copy postinstall script
  await copyFile(
    "scripts/postinstall-platform.mjs",
    join(distDir, "postinstall.mjs"),
  );
...
async function main() {
  ...
  if (process.argv.includes("--current-platform")) {
    ...
    platformsToBuild = platforms.filter(
  ...
  if (process.argv.includes("--pack")) {
    ...
    const packages = [
      "kuuzuki",
      ...platforms.map((p) => `kuuzuki-${p.os}-${p.arch}`),
    ];
