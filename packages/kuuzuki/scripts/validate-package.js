...
try {
  ...
  if (packageJson.type === "module") {
    warnings.push(
      'Package uses "type": "module" - ensure all scripts are ESM compatible',
    );
  }
...
console.log("\n🚀 Testing bin script...");
try {
  execSync("node bin/kuuzuki.js --help 2>&1", {
    cwd: join(__dirname, ".."),
    encoding: "utf8",
    stdio: "pipe",
  });
  ...
  // Check if it's our expected error message or development environment issues
  ...
  if (output.includes("v0.1.x currently supports TUI mode only")) {
    ...
    warnings.push(
      "Bin script has dependency issues in development environment - should work in published package",
    );
    console.log(
      "  ⚠️  Bin script has dev environment dependency issues (expected)",
    );
  } else {
...
if (existsSync(workflowPath)) {
  ...
  if (
    workflow.includes("mkdir -p bin") &&
    workflow.includes("cat > bin/kuuzuki.js")
  ) {
    warnings.push(
      "Workflow creates bin/kuuzuki.js - ensure it doesn't conflict with existing file",
    );
  }
...
// 5. Run npm pack dry run with timeout
console.log("\n📦 Testing npm pack...");
try {
  // Use timeout to prevent hanging in monorepo environments
  const output = execSync("timeout 30 npm pack --dry-run . 2>&1", {
    cwd: join(__dirname, ".."),
    encoding: "utf8",
    stdio: "pipe",
  });
  ...
  const kuuzukiSection = output
    .split("📦")
    .find((section) => section.includes("kuuzuki@"));
  ...
  if (kuuzukiSection && kuuzukiSection.includes(" src/")) {
    warnings.push(
      "Package includes src/ directory - consider if this is intended",
    );
  }
  ...
  if (e.status === 124) {
    warnings.push(
      "npm pack timed out (30s) - likely due to large node_modules in monorepo",
    );
    console.log(
      "  ⚠️  npm pack timed out - skipping detailed package validation",
    );
  } else {
