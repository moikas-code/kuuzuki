...
describe("Provider", () => {
  ...
  describe("getModel", () => {
    ...
    test("should throw ModelNotFoundError for unknown model", async () => {
      await App.provide({ cwd: process.cwd() }, async () => {
        await expect(
          Provider.getModel("anthropic", "unknown-model"),
    ...
    test("should return configured small model", async () => {
        ...
        return {
          ...
          Provider: {
            ...
            getModel: mock(async (providerID: string, modelID: string) => {
              ...
              throw new originalModule.Provider.ModelNotFoundError({
                providerID,
                modelID,
              });
            }),
      ...
      try {
        await App.provide({ cwd: process.cwd() }, async () => {
          ...
          expect(result?.info.id).toBe("claude-3-5-haiku");
        });
      } finally {
        // Cleanup
          ...
          Provider: {
            ...
            getSmallModel: async (providerID: string) => {
              if (providerID === "anthropic") {
                return {
                  info: { id: "claude-3-5-haiku", name: "Claude 3.5 Haiku" },
                  languageModel: {
                    id: "claude-3-5-haiku",
                    name: "Claude 3.5 Haiku",
                  },
