...
describe("Monitor", () => {
  beforeEach(async () => {
    ...
    Monitor.reset() // Clear metrics between tests
  })

  afterEach(async () => {
    Monitor.reset() // Clear metrics after tests
    await Monitor.shutdown()
  })
  ...
  describe("Performance namespace", () => {
    ...
    test("should record errors", () => {
      // Record a request first, then an error
  ...
  describe("High-level utilities", () => {
    ...
    test("should measure sync operations", () => {
      const result = Monitor.measureSync("sync-test", () => {
        ...
        // Busy wait for at least 1ms to ensure measurable time
      ...
      expect(result).toBeGreaterThan(0) // Result will vary based on CPU speed
  ...
  describe("Statistics", () => {
    test("should get monitor statistics", async () => {
      // Add a small delay to ensure uptime > 0
      await new Promise(resolve => setTimeout(resolve, 1))
  ...
  describe("performance thresholds", () => {
    test("should detect slow operations", async () => {
      ...
      console.warn = mock((message: string) => {
        if (message.toLowerCase().includes("slow operation")) {
