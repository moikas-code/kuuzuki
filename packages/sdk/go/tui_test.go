...
func TestTuiAppendPrompt(t *testing.T) {
	...
	_, err := client.Tui.AppendPrompt(context.TODO(), kuuzuki.TuiAppendPromptParams{
		Text: kuuzuki.F("text"),
	})
	if err != nil {
...
func TestTuiExecuteCommand(t *testing.T) {
	...
	_, err := client.Tui.ExecuteCommand(context.TODO(), kuuzuki.TuiExecuteCommandParams{
		Command: kuuzuki.F("command"),
	})
	if err != nil {
		var apierr *kuuzuki.Error
		if errors.As(err, &apierr) {
