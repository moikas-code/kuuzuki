...
interface DaemonConfig {
  ...
  beastMode: boolean; // Enable aggressive processing mode
  beastModeMaxChanges: number; // Max changes per run in beast mode
  beastModeAutoMergeThreshold: number; // Higher threshold for auto-merge in beast mode
...
class UpstreamSyncDaemon {
  ...
  constructor() {
    ...
    // Store the main repository path before we potentially change to worktree
    try {
      this.mainRepoPath = execSync("git rev-parse --show-toplevel", {
        encoding: "utf8",
      })
        .toString()
        .trim();
    } catch {
      this.mainRepoPath = process.cwd(); // fallback
    }
  }
  ...
  private async setupWorktree(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        execSync(`git branch ${daemonBranch} ${this.config.localBranch}`, {
          stdio: "pipe",
        });
      ...
      execSync(`git worktree add ${worktreePath} ${daemonBranch}`, {
        stdio: "pipe",
      });
  ...
  private async checkGitSetup(): Promise<boolean> {
    try {
      ...
      execSync(`git ls-remote ${this.config.upstreamRemote}`, {
        stdio: "pipe",
      });
  ...
  private async fetchUpstream(): Promise<void> {
    try {
      ...
      this.execInWorktree(`git fetch ${this.config.upstreamRemote}`, {
        stdio: "pipe",
      });
    } catch (error) {
  ...
  private async getNewCommits(): Promise<UpstreamChange[]> {
    try {
      ...
      // Use beast mode settings if enabled
      const maxChanges = this.config.beastMode
        ? this.config.beastModeMaxChanges
      ...
      // Reverse the commits to process them in chronological order (oldest first)
      // This ensures we don't accidentally degrade the project by applying newer changes before their dependencies
...
Analyze this upstream commit with subagents for integration into our fork:
...
Respond in JSON format:
{
  ...
  private createFallbackAnalysis(
    ...
    const shouldSkip = skipPatterns.some((pattern) =>
      pattern.test(change.message),
    );

    let type: "feature" | "bugfix" | "refactor" | "docs" | "test" | "chore" =
      "chore";
    ...
    if (change.files.length > 10 || change.additions + change.deletions > 500)
      risk = "critical";
    else if (
      ...
      change.additions + change.deletions > 20
    )
      risk = "medium";
    ...
    // Skip commits that are mostly excluded files (>80% excluded)
      ...
      confidence = 0.9;
      this.log(
        `🚫 Skipping commit with mostly excluded files: ${excludedFiles.length}/${change.files.length} files excluded`,
      );
    }
    // Skip commits that mostly affect files that don't exist in our fork (even with path translation)
      ...
      for (const file of change.files) {
        ...
        try {
          execSync(`git show ${this.config.localBranch}:${translatedPath}`, {
            stdio: "pipe",
          });
          existingFiles++;
        } catch {
      ...
      if (existingFiles === 0) {
        ...
        confidence = 0.9;
        this.log(
          `🚫 Skipping - no files exist in our fork (even with path translation): ${change.files.join(", ")}`,
        );
      }
    }
    // Special handling for bug fixes - they're usually important
    else if (type === ("bugfix" as const)) {
      if (risk === "low") {
    ...
    else if (hasExcluded && type !== ("bugfix" as const)) {
      recommendation = "skip";
      confidence = 0.9;
    }
    // Skip if not relevant AND not a bug fix
    else if (!isRelevant && type !== ("bugfix" as const)) {
      recommendation = "skip";
      confidence = 0.8;
    }
    ...
    this.log(
      `📊 Analysis for ${change.commit.substring(0, 8)}: ${type}/${risk} → ${recommendation} (${Math.round(confidence * 100)}%)`,
    );
    this.log(`   Files: ${change.files.join(", ")}`);
    this.log(
      `   Relevant: ${relevantFiles.length}/${change.files.length}, Excluded: ${excludedFiles.length}/${change.files.length} (${Math.round(excludedRatio * 100)}%)`,
    );
  ...
  private async isChangeAlreadyImplemented(
    change: UpstreamChange,
  ): Promise<boolean> {
    try {
      // Run implementation checks in the main repository, not the worktree
      const currentCwd = process.cwd();
      // Use the stored main repository path
      const mainRepoPath = this.mainRepoPath;

      this.log(
        `🔍 Implementation detection: currentCwd=${currentCwd}, mainRepoPath=${mainRepoPath}`,
      );
      this.log(
        `🔍 Change data: commit=${change.commit}, files=[${change.files.join(", ")}], message="${change.message}"`,
      );

      // Method 1: Check if commit exists in local branch (direct cherry-pick)
      try {
        execSync(
          `git merge-base --is-ancestor ${change.commit} ${this.config.localBranch}`,
          {
            stdio: "pipe",
            cwd: mainRepoPath,
          },
        ...
        this.log(
          `🔍 Commit ${change.commit.substring(0, 8)} already exists in ${this.config.localBranch}`,
        );
        return true;
      } catch {
      ...
      if (logOutput.trim()) {
        this.log(
          `🔍 Similar commit message found in ${this.config.localBranch}: ${change.message}`,
        );
        return true;
      }
      ...
      if (change.files.length > 0 && change.files.length <= 5) {
        ...
        if (allChangesPresent) {
          this.log(
            `🔍 File contents already match upstream for ${change.commit.substring(0, 8)}`,
          );
          return true;
        }
      }
      ...
      if (change.files.length > 0 && change.files.length <= 5) {
        ...
        try {
          const diff = execSync(`git show ${change.commit} --format=format:`, {
            encoding: "utf8",
            cwd: mainRepoPath,
          });
          ...
          if (addedLines.length > 0) {
            ...
            this.log(
              `🔍 Diff-based check: found ${foundChanges}/${addedLines.length} lines (${Math.round((foundChanges / addedLines.length) * 100)}%), need ${Math.ceil(addedLines.length * 0.7)}`,
            );
            if (foundChanges >= addedLines.length * 0.7) {
              this.log(
                `🔍 Specific changes already present in translated files (${foundChanges}/${addedLines.length} lines found)`,
              );
              return true;
            }
          }
          ...
          this.log(
            `⚠️  Error analyzing diff for implementation check: ${diffError}`,
          );
        }
      }
    ...
    try {
      ...
      for (const file of change.files) {
        try {
          ...
          execSync(`git show ${this.config.localBranch}:${file}`, {
            stdio: "pipe",
          });
          existingFiles++;
          ...
          if (recentChanges.trim()) {
            this.log(
              `⚠️  File ${file} has recent changes in ${this.config.localBranch}`,
            );
            return true;
          }
        } catch {
      ...
      if (missingFiles > existingFiles) {
        this.log(
          `⚠️  ${missingFiles}/${change.files.length} files don't exist in our fork - likely not applicable`,
        );
        return true;
      }
      ...
      if (change.files.includes("package.json")) {
        try {
          const upstreamPackage = execSync(
            `git show ${change.commit}:package.json`,
            { encoding: "utf8" },
          );
          const localPackage = execSync(
            `git show ${this.config.localBranch}:package.json`,
            { encoding: "utf8" },
          );
          ...
          if (upstreamVersion !== localVersion) {
            this.log(
              `⚠️  Version conflict: upstream ${upstreamVersion} vs local ${localVersion}`,
            );
            return true;
          }
        } catch {
    ...
    try {
      ...
      for (const file of change.files) {
        ...
        if (addedLines.length <= 5 && removedLines.length <= 5) {
          for (
            let i = 0;
            i < Math.min(addedLines.length, removedLines.length);
            i++
          ) {
            ...
            if (oldLine && modifiedContent.includes(oldLine)) {
              ...
              this.log(
                `🔄 Replaced line in ${translatedPath}: "${oldLine}" → "${newLine}"`,
              );
            }
          }
          ...
          if (addedLines.length > removedLines.length) {
            ...
            this.log(
              `➕ Added ${additionalLines.length} lines to ${translatedPath}`,
            );
          }
        }
  ...
  private async smartCherryPick(change: UpstreamChange): Promise<boolean> {
    try {
      ...
      try {
        ...
        this.log(
          `✅ Regular cherry-pick succeeded for ${change.commit.substring(0, 8)}`,
        );
        return true;
        ...
        try {
          ...
          this.log(
            `⚠️  Cherry-pick abort failed (might not be in progress): ${abortError}`,
          );
        }
      }
      ...
      const patch = execSync(`git show ${change.commit} --format=format:`, {
        encoding: "utf8",
      });

      // Translate paths in the patch
      let translatedPatch = patch;
      for (const [upstreamPath, localPath] of Object.entries(
        this.config.pathMappings,
      )) {
        const regex = new RegExp(
          upstreamPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
          "g",
        );
      ...
      try {
        ...
        try {
          ...
          this.execInWorktree(`git apply --3way ${tempPatchFile}`, {
            stdio: "pipe",
          });
          ...
          try {
            const status = this.execInWorktree(`git status --porcelain`, {
              encoding: "utf8",
            }) as string;
            const conflictedFiles = status
              .split("\n")
              .filter(
                (line) => line.startsWith("UU ") || line.startsWith("U "),
              );

            if (conflictedFiles.length > 0 && conflictedFiles.length <= 3) {
              this.log(
                `🔧 Found ${conflictedFiles.length} conflicted files, attempting resolution...`,
              );

              // For review branches, we can leave conflicts for manual resolution
              // Just stage the non-conflicted files and create a commit
              const cleanFiles = status
                .split("\n")
                .filter(
                  (line) =>
              ...
              if (cleanFiles.length > 0) {
                this.log(
                  `✅ Applied ${cleanFiles.length} files cleanly, ${conflictedFiles.length} need manual resolution`,
                );
          ...
          if (!patchApplied) {
            try {
              ...
              this.execInWorktree(
                `git apply --ignore-whitespace --ignore-space-change ${tempPatchFile}`,
                { stdio: "pipe" },
              );
              ...
              this.log(
                `⚠️  Whitespace-tolerant patch failed: ${whitespaceError}`,
              );
              ...
              if (
                ...
                this.log(
                  `🔧 Attempting manual application for small change...`,
                );
      ...
      if (!patchApplied) {
        throw new Error(
          `Failed to apply translated patch after trying multiple strategies`,
        );
      }
  ...
  private async integrateChange(change: UpstreamChange): Promise<boolean> {
    ...
    if (recommendation === "skip") {
      this.log(
        `⏭️  Skipping ${change.commit.substring(0, 8)}: ${change.message}`,
      );
      this.markCommitProcessed(
        change.commit,
        "skipped",
    ...
    if (recommendation === "manual") {
      this.log(
        `⚠️  Manual review required for ${change.commit.substring(0, 8)}: ${change.message}`,
      );
      this.markCommitProcessed(
        change.commit,
        "manual-required",
    ...
    // Use beast mode threshold if enabled
    const threshold = this.config.beastMode
      ? this.config.beastModeAutoMergeThreshold
      : this.config.autoMergeThreshold;
    if (recommendation === "auto-merge" && confidence >= threshold) {
      return await this.performIntegration(change);
    }

    if (recommendation === "review") {
      this.log(
        `👀 Review recommended for ${change.commit.substring(0, 8)}: ${change.message}`,
      );
      if (this.config.dryRun) {
        this.log(
          `🔍 [DRY RUN] Would create review branch for manual inspection`,
        );
        return true;
      }
  ...
  private async performIntegration(change: UpstreamChange): Promise<boolean> {
    try {
      this.log(
        `🔄 Auto-integrating ${change.commit.substring(0, 8)}: ${change.message}`,
      );
      ...
      if (await this.hasLikelyConflicts(change)) {
        this.log(
          `⚠️  Potential conflicts detected for ${change.commit.substring(0, 8)}, creating review branch instead`,
        );
        return await this.createReviewBranch(change);
      }
      ...
      try {
        ...
        if (testsPassed) {
          ...
          this.log(
            `✅ Successfully integrated ${change.commit.substring(0, 8)}`,
          );
          this.markCommitProcessed(
            change.commit,
            "auto-merged",
            "Successfully auto-integrated",
          );
          return true;
        } else {
          ...
          execSync(`git branch -D ${branchName}`, { stdio: "pipe" });
          this.log(
            `❌ Integration failed tests for ${change.commit.substring(0, 8)}`,
          );
          this.markCommitProcessed(
            change.commit,
            "failed",
            "Integration failed tests",
          );
          return false;
        }
      } catch (cherryPickError) {
        ...
        this.log(
          `❌ Cherry-pick failed for ${change.commit.substring(0, 8)}: ${cherryPickError}`,
        );
        this.markCommitProcessed(
          change.commit,
          "failed",
          `Cherry-pick failed: ${cherryPickError}`,
        );
        return false;
      }
    } catch (error) {
      this.log(
        `❌ Integration error for ${change.commit.substring(0, 8)}: ${error}`,
      );
      return false;
    }
  }
  ...
  private async createReviewBranch(change: UpstreamChange): Promise<boolean> {
    try {
      ...
      try {
        ...
        this.log(
          `   To integrate: git checkout ${this.config.localBranch} && git merge ${branchName}`,
        );
        ...
        this.log(
          `❌ Cherry-pick failed for review branch ${branchName}: ${cherryPickError}`,
        );
        this.markCommitProcessed(
          change.commit,
          "failed",
          `Review branch creation failed: ${cherryPickError}`,
        );
        return false;
      }
    } catch (error) {
  ...
  private async syncCycle(): Promise<void> {
    try {
      ...
      this.log(
        `📦 Found ${newCommits.length} new commits (processing chronologically: oldest → newest)`,
      );
      ...
      for (const commit of newCommits) {
        ...
        if (this.isCommitProcessed(commit.commit)) {
          ...
          this.log(
            `⏭️  Skipping already processed commit ${commit.commit.substring(0, 8)}: ${processed.status} (${processed.reason})`,
          );
          skipped++;
          continue;
        }

        // Check if this change is already implemented BEFORE doing AI analysis
        this.log(
          `🔍 Checking if ${commit.commit.substring(0, 8)} is already implemented...`,
        );
        if (await this.isChangeAlreadyImplemented(commit)) {
          this.log(
            `✅ Already implemented ${commit.commit.substring(0, 8)}: ${commit.message}`,
          );
          this.markCommitProcessed(
            commit.commit,
            "already-implemented",
            "Change already exists in codebase",
          );
          skipped++;
          continue;
        }
        this.log(
          `❌ Not detected as already implemented, proceeding with AI analysis...`,
        );
      ...
      this.log(
        `📊 Sync complete: ${integrated} integrated, ${skipped} skipped, ${failed} failed`,
      );

      if (this.config.notifyOnChanges && (integrated > 0 || failed > 0)) {
        this.sendNotification(
          `Upstream sync: ${integrated} integrated, ${failed} failed`,
        );
      }
    } catch (error) {
  ...
  private sendNotification(message: string): void {
    ...
    try {
      execSync(`notify-send "Kuuzuki Upstream Sync" "${message}"`, {
        stdio: "pipe",
      });
    } catch (error) {
  ...
  public async start(): Promise<void> {
    ...
    this.log(
      `   Remote: ${this.config.upstreamRemote}/${this.config.upstreamBranch}`,
    );
    ...
    this.log(
      `   Worktree: ${this.config.useWorktree ? this.config.worktreePath : "disabled"}`,
    );
    if (this.config.beastMode) {
      this.log(
        `   🦁 Beast Mode: ${this.config.beastModeMaxChanges} max changes, ${this.config.beastModeAutoMergeThreshold} auto-merge threshold`,
      );
    }
  ...
  public status(): void {
    ...
    if (existsSync(TRACKER_FILE)) {
      ...
      console.log(
        `   Last processed: ${tracker.lastProcessedCommit?.substring(0, 8) || "none"}`,
      );

      const statusCounts = commits.reduce(
        (acc, commit) => {
      ...
      if (recent.length > 0) {
        ...
        recent.forEach((commit) => {
          ...
          console.log(
            `   ${emoji} ${commit.commit.substring(0, 8)} (${commit.status}) - ${date}`,
          );
          if (commit.branch) {
